"""
v39: Differential Attention (Ye et al., 2024) — split Q/K heads into sub-head pairs,
compute two softmax attentions, subtract with learnable lambda to cancel attention noise.
FA3 on Hopper supports headdim_qk != headdim_v, so Q/K at D//2 with V at full D.
"""
...
class Hyperparameters:
    ...
    diff_attn_lambda_init = float(os.environ.get("DIFF_ATTN_LAMBDA_INIT", 0.8))
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        # Differential Attention: learnable lambda per head, clamped to [0, 1.5]
        self.lambda_param = nn.Parameter(torch.full((num_heads,), diff_attn_lambda_init, dtype=torch.float32))
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        # RMS norm on full Q, K before splitting
        q = F.rms_norm(q, (q.size(-1),))
        k = F.rms_norm(k, (k.size(-1),))
        # Differential Attention: split Q, K into two sub-heads along head_dim
        sub_dim = self.head_dim // 2
        q1, q2 = q[..., :sub_dim], q[..., sub_dim:]
        k1, k2 = k[..., :sub_dim], k[..., sub_dim:]
        # Apply RoPE to each sub-head pair (rope_dims=16 rotates 16/32 dims per sub-head)
        cos, sin = self.rotary(seqlen, x.device, q.dtype)
        q1 = apply_rotary_emb(q1.contiguous(), cos, sin, self.rope_dims)
        k1 = apply_rotary_emb(k1.contiguous(), cos, sin, self.rope_dims)
        q2 = apply_rotary_emb(q2.contiguous(), cos, sin, self.rope_dims)
        k2 = apply_rotary_emb(k2.contiguous(), cos, sin, self.rope_dims)
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, diff_attn_lambda_init)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        diff_attn_lambda_init=args.diff_attn_lambda_init,
    ).to(device).bfloat16()
    ...
    for block in base_model.blocks:
        scalar_params.append(block.attn.lambda_param)
